...
class c_testtb extends base_controller
{
    ...
    public function list_index()
    {
        try
        {
            ...
            //<!--列表头部ctl数据--><!--903--><!--该代码由系统自动生成-->
            if (($username = $_GET['username']) != "") //用户名 列表检索
            {
                $conditions .= " AND username like '%$username%'";
                $urls .= "&username=$username";
            }
            if (($nickname = $_GET['nickname']) != "") //昵称 列表检索
            {
                $conditions .= " AND nickname like '%$nickname%'";
                $urls .= "&nickname=$nickname";
            }
            $status = isset($_GET["status"]) && in_array($_GET["status"], array(1,0)) ? $_GET["status"] : "";
            if (($status = $_GET['status']) != "") //帐号状态，=0为禁用，=1为正常，默认为=1 列表检索
            {
            ...
            if($_SERVER["REQUEST_METHOD"] == "POST")
            {
                ...
                else //添加
                {
                    ...
                    $urllist = array(
                        array(
                            'txt' => "继续添加",
                            'url' => U("testtb/edit?parent=".$_POST['parent']),
                        ),
                        array(
                            'txt' => "返回列表页面",
                            'url' => U("testtb"),
