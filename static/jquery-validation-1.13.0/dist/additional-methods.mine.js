...
$(function(){
	...
	//正整数验证 使用示例：uint32:true
	$.validator.addMethod("uint32", function(value, element) {
		return this.optional(element) || (/^-?\d+$/.test(value) && value >= 0);
	}, "请输入正整数");

	//在min-max之间的整数 使用示例：uint32Range:[2,3]
	$.validator.addMethod("uint32Range", function(value, element, param) {
		return this.optional(element) || (/^-?\d+$/.test(value) && value >= param[ 0 ] && value <= param[ 1 ] );
	}, "请输入{0}-{1}的整数");

	//在枚举范围内  使用示例：enum:[2,5,'a']
