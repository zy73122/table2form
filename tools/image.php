...
//define('PATH_ROOT',					 '网站根目录');

/* image类的语言项 */
$GLOBALS['config']['_LANG']['directory_readonly'] = '目录 %s 不存在或不可写';
$GLOBALS['config']['_LANG']['invalid_upload_image_type'] = '不是允许的图片格式';
$GLOBALS['config']['_LANG']['upload_failure'] = '文件 %s 上传失败。';
$GLOBALS['config']['_LANG']['missing_gd'] = '没有安装GD库';
$GLOBALS['config']['_LANG']['missing_orgin_image'] = '找不到原始图片 %s ';
$GLOBALS['config']['_LANG']['nonsupport_type'] = '不支持该图像格式 %s ';
$GLOBALS['config']['_LANG']['creating_failure'] = '创建图片失败';
$GLOBALS['config']['_LANG']['writting_failure'] = '图片写入失败';
$GLOBALS['config']['_LANG']['empty_watermark'] = '水印文件参数不能为空';
$GLOBALS['config']['_LANG']['missing_watermark'] = '找不到水印文件%s';
$GLOBALS['config']['_LANG']['create_watermark_res'] = '创建水印图片资源失败。水印图片类型为%s';
$GLOBALS['config']['_LANG']['create_origin_image_res'] = '创建原始图片资源失败，原始图片类型%s';
$GLOBALS['config']['_LANG']['invalid_image_type'] = '无法识别水印图片 %s ';
$GLOBALS['config']['_LANG']['file_unavailable'] = '文件 %s 不存在或不可读';
...
class image
{
	...
	var $current_full_filename = '';
	var $images_dir  = 'data/upload/images';
	var $thumb_dir	 = 'data/upload/thumb';
	var $data_dir	= 'data/upload';
	var $bgcolor	 = '';
	...
	function upload_image($upload, $dir = '', $img_name = '')
	{
		...
		if (empty($dir))
		{
			/* 创建当月目录 */
			$dir = date('Ym');
			$dir = PATH_ROOT . $this->images_dir . '/' . $dir . '/';
		}
		else
		{
			/* 创建目录 */
			$dir = PATH_ROOT . $this->data_dir . '/' . $dir . '/';
			if ($img_name)
			{
		...
		if (!file_exists($dir))
		{
			if (!cfile::mkdir_recursive($dir, 0700))
			{
				/* 创建目录失败 */
				$this->error_msg = sprintf($GLOBALS['config']['_LANG']['directory_readonly'], $dir);
		...
		if (empty($img_name))
		{
			...
			$this->current_filename = $img_name;
			$img_name = $dir . $img_name . $this->get_filetype($upload['name']);
		}

		if (!$this->check_img_type($upload['type']))
		{
			$this->error_msg = $GLOBALS['config']['_LANG']['invalid_upload_image_type'];
		...
		{
			return str_replace(PATH_ROOT, '', $img_name);
		}
		else
		{
			$this->error_msg = sprintf($GLOBALS['config']['_LANG']['upload_failure'], $upload['name']);
	...
	function make_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='')
	{
		...
		if ($gd == 0)
		{
			$this->error_msg = $GLOBALS['config']['_LANG']['missing_gd'];
			return false;
		}
		...
		{
			return str_replace(PATH_ROOT, '', str_replace('\\', '/', realpath($img)));
		}
		...
		if (!$org_info)
		{
			$this->error_msg = sprintf($GLOBALS['config']['_LANG']['missing_orgin_image'], $img);
		...
		{
			$this->error_msg = sprintf($GLOBALS['config']['_LANG']['nonsupport_type'], $this->type_maping[$org_info[2]]);
		...
		if ($thumb_width>$original_width && $thumb_height>$original_height)
		{
		...
		if (empty($path))
		{
			$dir = PATH_ROOT . $this->thumb_dir . '/' . date('Ym').'/';
		}
		else
		{
			$dir = $path;
		...
		{
			$this->error_msg = $GLOBALS['config']['_LANG']['creating_failure'];
			$this->error_no  =  ERR_NO_GD;
		...
		{
			return str_replace(PATH_ROOT, '', $dir) . $filename;
		}
		else
		{
			$this->error_msg = $GLOBALS['config']['_LANG']['writting_failure'];
			$this->error_no   = ERR_DIRECTORY_READONLY;
	 ...
	 */
	function add_watermark($filename, $target_file='', $watermark='', $watermark_place='', $watermark_alpha = 65)
	{
		// 是否安装了GD
		$gd = $this->gd_version();
		if ($gd == 0)
		...
		{
			$this->error_msg  = sprintf($GLOBALS['config']['_LANG']['missing_orgin_image'], $filename);
			$this->error_no   = ERR_IMAGE_NOT_EXISTS;
		...
		{
			return str_replace(PATH_ROOT, '', str_replace('\\', '/', realpath($filename)));
		}
		...
		if (!$watermark_handle)
		{
			$this->error_msg = sprintf($GLOBALS['config']['_LANG']['create_watermark_res'], $this->type_maping[$watermark_info[2]]);
		...
		if (!$source_handle)
		{
			$this->error_msg = sprintf($GLOBALS['config']['_LANG']['create_origin_image_res'], $this->type_maping[$source_info[2]]);
			$this->error_no = ERR_INVALID_IMAGE;
		...
		if ($path)
		{
			return str_replace(str_replace('\\', '/', PATH_ROOT), '', str_replace('\\', '/', $path));
		}
		else
		{
			$this->error_msg = $GLOBALS['config']['_LANG']['writting_failure'];
	...
	function validate_image($path)
	{
		if (empty($path))
		{
			$this->error_msg = $GLOBALS['config']['_LANG']['empty_watermark'];
		...
		if (!file_exists($path))
		{
			$this->error_msg = sprintf($GLOBALS['config']['_LANG']['missing_watermark'], $path);
			$this->error_no = ERR_IMAGE_NOT_EXISTS;
			return false;
		}
		...
		if (!$image_info)
		{
			$this->error_msg = sprintf($GLOBALS['config']['_LANG']['invalid_image_type'], $path);
			$this->error_no = ERR_INVALID_IMAGE;
			return false;
		}
		...
		{
			$this->error_msg = sprintf($GLOBALS['config']['_LANG']['nonsupport_type'], $this->type_maping[$image_info[2]]);
			$this->error_no  =  ERR_NO_GD;
			return false;
		}
